 ...
 * 
 */
public class SoaBinding implements ISoaBinding {
  ...
  public Object invoke(Object target, String action, AttrAccess inparms) {
    ...
    if (toinvoke.getAnnotation(SoaAllParms.class) != null) {
      ...
      for (String arg : mdesc.args()) {
        aa.setAttr(arg);
        if (!aa.isPopulated(arg))
          aa.setAttr(arg, ""); // BUG null values are NOT populated
      }

      for (String arg : aa.getPopulatedAttr()) {
        Object a = inparms.getAttr(arg);
        if (a == null)
          a = aa.getAttr(arg); // default value?
        args.add(a);
      }
    }
  ...
  public Object execServer(String actionName, String protocol, String cmdargs, Properties parms,
    ...
    throw new UnsupportedOperationException("must implement this in derived classes");
  }
